...
class DoneFragment : Fragment() {
    ...
    private fun initListeners() {
        binding.floatingActionButton2.setOnClickListener {
            val action = HomeFragmentDirections
                .actionHomeFragmentToFormTaskFragment(null, "done")
    ...
    private fun observerViewModel() {
        viewModel.taskList.observe(viewLifecycleOwner) { fullList ->
            val filtered = fullList.filter { it.status == Status.DONE }
    ...
    private fun optionSelected(task: Task, option: Int) {
        when (option) {
            ...
            TaskAdapter.SELECT_BACK -> {
                moveTaskToPreviousStatus(task)  // Mover para DOING
            }

            TaskAdapter.SELECT_NEXT -> {
                moveTaskToNextStatus(task)  // Mover para TODO
            }
        }
    }

    private fun moveTaskToNextStatus(task: Task) {
        // Lógica de mover para o próximo status na sequência: TODO -> DOING -> DONE -> TODO
        val nextStatus = when (task.status) {
            Status.TODO -> Status.DOING
            Status.DOING -> Status.DONE
            Status.DONE -> Status.TODO  // Voltar para TODO
        }
    ...
    private fun moveTaskToPreviousStatus(task: Task) {
        // Lógica de mover para o status anterior na sequência: TODO <- DOING <- DONE <- TODO
        val previousStatus = when (task.status) {
            Status.TODO -> Status.DONE
            Status.DOING -> Status.TODO
            Status.DONE -> Status.DOING  // Voltar para DOING
        }
